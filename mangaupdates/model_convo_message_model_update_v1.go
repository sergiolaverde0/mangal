/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ConvoMessageModelUpdateV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConvoMessageModelUpdateV1{}

// ConvoMessageModelUpdateV1 struct for ConvoMessageModelUpdateV1
type ConvoMessageModelUpdateV1 struct {
	Content *string `json:"content,omitempty"`
}

// NewConvoMessageModelUpdateV1 instantiates a new ConvoMessageModelUpdateV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConvoMessageModelUpdateV1() *ConvoMessageModelUpdateV1 {
	this := ConvoMessageModelUpdateV1{}
	return &this
}

// NewConvoMessageModelUpdateV1WithDefaults instantiates a new ConvoMessageModelUpdateV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConvoMessageModelUpdateV1WithDefaults() *ConvoMessageModelUpdateV1 {
	this := ConvoMessageModelUpdateV1{}
	return &this
}

// GetContent returns the Content field value if set, zero value otherwise.
func (o *ConvoMessageModelUpdateV1) GetContent() string {
	if o == nil || IsNil(o.Content) {
		var ret string
		return ret
	}
	return *o.Content
}

// GetContentOk returns a tuple with the Content field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvoMessageModelUpdateV1) GetContentOk() (*string, bool) {
	if o == nil || IsNil(o.Content) {
		return nil, false
	}
	return o.Content, true
}

// HasContent returns a boolean if a field has been set.
func (o *ConvoMessageModelUpdateV1) HasContent() bool {
	if o != nil && !IsNil(o.Content) {
		return true
	}

	return false
}

// SetContent gets a reference to the given string and assigns it to the Content field.
func (o *ConvoMessageModelUpdateV1) SetContent(v string) {
	o.Content = &v
}

func (o ConvoMessageModelUpdateV1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConvoMessageModelUpdateV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Content) {
		toSerialize["content"] = o.Content
	}
	return toSerialize, nil
}

type NullableConvoMessageModelUpdateV1 struct {
	value *ConvoMessageModelUpdateV1
	isSet bool
}

func (v NullableConvoMessageModelUpdateV1) Get() *ConvoMessageModelUpdateV1 {
	return v.value
}

func (v *NullableConvoMessageModelUpdateV1) Set(val *ConvoMessageModelUpdateV1) {
	v.value = val
	v.isSet = true
}

func (v NullableConvoMessageModelUpdateV1) IsSet() bool {
	return v.isSet
}

func (v *NullableConvoMessageModelUpdateV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConvoMessageModelUpdateV1(val *ConvoMessageModelUpdateV1) *NullableConvoMessageModelUpdateV1 {
	return &NullableConvoMessageModelUpdateV1{value: val, isSet: true}
}

func (v NullableConvoMessageModelUpdateV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConvoMessageModelUpdateV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


