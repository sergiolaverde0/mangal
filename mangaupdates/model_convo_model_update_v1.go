/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ConvoModelUpdateV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConvoModelUpdateV1{}

// ConvoModelUpdateV1 struct for ConvoModelUpdateV1
type ConvoModelUpdateV1 struct {
	Topic *string `json:"topic,omitempty"`
}

// NewConvoModelUpdateV1 instantiates a new ConvoModelUpdateV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConvoModelUpdateV1() *ConvoModelUpdateV1 {
	this := ConvoModelUpdateV1{}
	return &this
}

// NewConvoModelUpdateV1WithDefaults instantiates a new ConvoModelUpdateV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConvoModelUpdateV1WithDefaults() *ConvoModelUpdateV1 {
	this := ConvoModelUpdateV1{}
	return &this
}

// GetTopic returns the Topic field value if set, zero value otherwise.
func (o *ConvoModelUpdateV1) GetTopic() string {
	if o == nil || IsNil(o.Topic) {
		var ret string
		return ret
	}
	return *o.Topic
}

// GetTopicOk returns a tuple with the Topic field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConvoModelUpdateV1) GetTopicOk() (*string, bool) {
	if o == nil || IsNil(o.Topic) {
		return nil, false
	}
	return o.Topic, true
}

// HasTopic returns a boolean if a field has been set.
func (o *ConvoModelUpdateV1) HasTopic() bool {
	if o != nil && !IsNil(o.Topic) {
		return true
	}

	return false
}

// SetTopic gets a reference to the given string and assigns it to the Topic field.
func (o *ConvoModelUpdateV1) SetTopic(v string) {
	o.Topic = &v
}

func (o ConvoModelUpdateV1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConvoModelUpdateV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Topic) {
		toSerialize["topic"] = o.Topic
	}
	return toSerialize, nil
}

type NullableConvoModelUpdateV1 struct {
	value *ConvoModelUpdateV1
	isSet bool
}

func (v NullableConvoModelUpdateV1) Get() *ConvoModelUpdateV1 {
	return v.value
}

func (v *NullableConvoModelUpdateV1) Set(val *ConvoModelUpdateV1) {
	v.value = val
	v.isSet = true
}

func (v NullableConvoModelUpdateV1) IsSet() bool {
	return v.isSet
}

func (v *NullableConvoModelUpdateV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConvoModelUpdateV1(val *ConvoModelUpdateV1) *NullableConvoModelUpdateV1 {
	return &NullableConvoModelUpdateV1{value: val, isSet: true}
}

func (v NullableConvoModelUpdateV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConvoModelUpdateV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


