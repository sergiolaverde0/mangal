/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the AboutusDescriptionModelV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AboutusDescriptionModelV1{}

// AboutusDescriptionModelV1 struct for AboutusDescriptionModelV1
type AboutusDescriptionModelV1 struct {
	Description *string `json:"description,omitempty"`
}

// NewAboutusDescriptionModelV1 instantiates a new AboutusDescriptionModelV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAboutusDescriptionModelV1() *AboutusDescriptionModelV1 {
	this := AboutusDescriptionModelV1{}
	return &this
}

// NewAboutusDescriptionModelV1WithDefaults instantiates a new AboutusDescriptionModelV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAboutusDescriptionModelV1WithDefaults() *AboutusDescriptionModelV1 {
	this := AboutusDescriptionModelV1{}
	return &this
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *AboutusDescriptionModelV1) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AboutusDescriptionModelV1) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *AboutusDescriptionModelV1) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *AboutusDescriptionModelV1) SetDescription(v string) {
	o.Description = &v
}

func (o AboutusDescriptionModelV1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AboutusDescriptionModelV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	return toSerialize, nil
}

type NullableAboutusDescriptionModelV1 struct {
	value *AboutusDescriptionModelV1
	isSet bool
}

func (v NullableAboutusDescriptionModelV1) Get() *AboutusDescriptionModelV1 {
	return v.value
}

func (v *NullableAboutusDescriptionModelV1) Set(val *AboutusDescriptionModelV1) {
	v.value = val
	v.isSet = true
}

func (v NullableAboutusDescriptionModelV1) IsSet() bool {
	return v.isSet
}

func (v *NullableAboutusDescriptionModelV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAboutusDescriptionModelV1(val *AboutusDescriptionModelV1) *NullableAboutusDescriptionModelV1 {
	return &NullableAboutusDescriptionModelV1{value: val, isSet: true}
}

func (v NullableAboutusDescriptionModelV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAboutusDescriptionModelV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


