/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the PollVoteStatusModelV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PollVoteStatusModelV1{}

// PollVoteStatusModelV1 struct for PollVoteStatusModelV1
type PollVoteStatusModelV1 struct {
	Voted *bool `json:"voted,omitempty"`
}

// NewPollVoteStatusModelV1 instantiates a new PollVoteStatusModelV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPollVoteStatusModelV1() *PollVoteStatusModelV1 {
	this := PollVoteStatusModelV1{}
	return &this
}

// NewPollVoteStatusModelV1WithDefaults instantiates a new PollVoteStatusModelV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPollVoteStatusModelV1WithDefaults() *PollVoteStatusModelV1 {
	this := PollVoteStatusModelV1{}
	return &this
}

// GetVoted returns the Voted field value if set, zero value otherwise.
func (o *PollVoteStatusModelV1) GetVoted() bool {
	if o == nil || IsNil(o.Voted) {
		var ret bool
		return ret
	}
	return *o.Voted
}

// GetVotedOk returns a tuple with the Voted field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PollVoteStatusModelV1) GetVotedOk() (*bool, bool) {
	if o == nil || IsNil(o.Voted) {
		return nil, false
	}
	return o.Voted, true
}

// HasVoted returns a boolean if a field has been set.
func (o *PollVoteStatusModelV1) HasVoted() bool {
	if o != nil && !IsNil(o.Voted) {
		return true
	}

	return false
}

// SetVoted gets a reference to the given bool and assigns it to the Voted field.
func (o *PollVoteStatusModelV1) SetVoted(v bool) {
	o.Voted = &v
}

func (o PollVoteStatusModelV1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PollVoteStatusModelV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Voted) {
		toSerialize["voted"] = o.Voted
	}
	return toSerialize, nil
}

type NullablePollVoteStatusModelV1 struct {
	value *PollVoteStatusModelV1
	isSet bool
}

func (v NullablePollVoteStatusModelV1) Get() *PollVoteStatusModelV1 {
	return v.value
}

func (v *NullablePollVoteStatusModelV1) Set(val *PollVoteStatusModelV1) {
	v.value = val
	v.isSet = true
}

func (v NullablePollVoteStatusModelV1) IsSet() bool {
	return v.isSet
}

func (v *NullablePollVoteStatusModelV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePollVoteStatusModelV1(val *PollVoteStatusModelV1) *NullablePollVoteStatusModelV1 {
	return &NullablePollVoteStatusModelV1{value: val, isSet: true}
}

func (v NullablePollVoteStatusModelV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePollVoteStatusModelV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


