/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the UserGenreHighlightModelV1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UserGenreHighlightModelV1{}

// UserGenreHighlightModelV1 struct for UserGenreHighlightModelV1
type UserGenreHighlightModelV1 struct {
	GenreId   *int64  `json:"genre_id,omitempty"`
	GenreName *string `json:"genre_name,omitempty"`
	Color     *string `json:"color,omitempty"`
}

// NewUserGenreHighlightModelV1 instantiates a new UserGenreHighlightModelV1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUserGenreHighlightModelV1() *UserGenreHighlightModelV1 {
	this := UserGenreHighlightModelV1{}
	return &this
}

// NewUserGenreHighlightModelV1WithDefaults instantiates a new UserGenreHighlightModelV1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUserGenreHighlightModelV1WithDefaults() *UserGenreHighlightModelV1 {
	this := UserGenreHighlightModelV1{}
	return &this
}

// GetGenreId returns the GenreId field value if set, zero value otherwise.
func (o *UserGenreHighlightModelV1) GetGenreId() int64 {
	if o == nil || IsNil(o.GenreId) {
		var ret int64
		return ret
	}
	return *o.GenreId
}

// GetGenreIdOk returns a tuple with the GenreId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserGenreHighlightModelV1) GetGenreIdOk() (*int64, bool) {
	if o == nil || IsNil(o.GenreId) {
		return nil, false
	}
	return o.GenreId, true
}

// HasGenreId returns a boolean if a field has been set.
func (o *UserGenreHighlightModelV1) HasGenreId() bool {
	if o != nil && !IsNil(o.GenreId) {
		return true
	}

	return false
}

// SetGenreId gets a reference to the given int64 and assigns it to the GenreId field.
func (o *UserGenreHighlightModelV1) SetGenreId(v int64) {
	o.GenreId = &v
}

// GetGenreName returns the GenreName field value if set, zero value otherwise.
func (o *UserGenreHighlightModelV1) GetGenreName() string {
	if o == nil || IsNil(o.GenreName) {
		var ret string
		return ret
	}
	return *o.GenreName
}

// GetGenreNameOk returns a tuple with the GenreName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserGenreHighlightModelV1) GetGenreNameOk() (*string, bool) {
	if o == nil || IsNil(o.GenreName) {
		return nil, false
	}
	return o.GenreName, true
}

// HasGenreName returns a boolean if a field has been set.
func (o *UserGenreHighlightModelV1) HasGenreName() bool {
	if o != nil && !IsNil(o.GenreName) {
		return true
	}

	return false
}

// SetGenreName gets a reference to the given string and assigns it to the GenreName field.
func (o *UserGenreHighlightModelV1) SetGenreName(v string) {
	o.GenreName = &v
}

// GetColor returns the Color field value if set, zero value otherwise.
func (o *UserGenreHighlightModelV1) GetColor() string {
	if o == nil || IsNil(o.Color) {
		var ret string
		return ret
	}
	return *o.Color
}

// GetColorOk returns a tuple with the Color field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UserGenreHighlightModelV1) GetColorOk() (*string, bool) {
	if o == nil || IsNil(o.Color) {
		return nil, false
	}
	return o.Color, true
}

// HasColor returns a boolean if a field has been set.
func (o *UserGenreHighlightModelV1) HasColor() bool {
	if o != nil && !IsNil(o.Color) {
		return true
	}

	return false
}

// SetColor gets a reference to the given string and assigns it to the Color field.
func (o *UserGenreHighlightModelV1) SetColor(v string) {
	o.Color = &v
}

func (o UserGenreHighlightModelV1) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UserGenreHighlightModelV1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.GenreId) {
		toSerialize["genre_id"] = o.GenreId
	}
	if !IsNil(o.GenreName) {
		toSerialize["genre_name"] = o.GenreName
	}
	if !IsNil(o.Color) {
		toSerialize["color"] = o.Color
	}
	return toSerialize, nil
}

type NullableUserGenreHighlightModelV1 struct {
	value *UserGenreHighlightModelV1
	isSet bool
}

func (v NullableUserGenreHighlightModelV1) Get() *UserGenreHighlightModelV1 {
	return v.value
}

func (v *NullableUserGenreHighlightModelV1) Set(val *UserGenreHighlightModelV1) {
	v.value = val
	v.isSet = true
}

func (v NullableUserGenreHighlightModelV1) IsSet() bool {
	return v.isSet
}

func (v *NullableUserGenreHighlightModelV1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUserGenreHighlightModelV1(val *UserGenreHighlightModelV1) *NullableUserGenreHighlightModelV1 {
	return &NullableUserGenreHighlightModelV1{value: val, isSet: true}
}

func (v NullableUserGenreHighlightModelV1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUserGenreHighlightModelV1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
