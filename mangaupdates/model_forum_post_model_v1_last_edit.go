/*
MangaUpdates API

This API powers our website. Most API functions are public and do not require an account. For user-based functions, you must register an account. Currently, user registration must be done through our main website and is disabled via this API.  Download OpenAPI Specification: [openapi.yaml](openapi.yaml)  Please contact us at the following emails if you have questions:  * lambchopsil@mangaupdates.com * manick@mangaupdates.com  ## Warranties  MangaUpdates makes no warranties about service availability, correctness of the data, or anything else. The service is provided as is, and may change at any time.  ## Acceptable Use Policy  * You will credit MangaUpdates when using data provided by this API. * You will use reasonable spacing between requests so as not to overwhelm the MangaUpdates servers, and employ caching mechanisms when accessing data. * You will NOT use MangaUpdates data or API in a way that will:     * Deceive or defraud users     * Assist or perform an illegal action     * Create spam     * Damage the database  We reserve the right to change this policy at any time.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ForumPostModelV1LastEdit type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ForumPostModelV1LastEdit{}

// ForumPostModelV1LastEdit struct for ForumPostModelV1LastEdit
type ForumPostModelV1LastEdit struct {
	By *string `json:"by,omitempty"`
	Time *TimeV1 `json:"time,omitempty"`
}

// NewForumPostModelV1LastEdit instantiates a new ForumPostModelV1LastEdit object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewForumPostModelV1LastEdit() *ForumPostModelV1LastEdit {
	this := ForumPostModelV1LastEdit{}
	return &this
}

// NewForumPostModelV1LastEditWithDefaults instantiates a new ForumPostModelV1LastEdit object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewForumPostModelV1LastEditWithDefaults() *ForumPostModelV1LastEdit {
	this := ForumPostModelV1LastEdit{}
	return &this
}

// GetBy returns the By field value if set, zero value otherwise.
func (o *ForumPostModelV1LastEdit) GetBy() string {
	if o == nil || IsNil(o.By) {
		var ret string
		return ret
	}
	return *o.By
}

// GetByOk returns a tuple with the By field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ForumPostModelV1LastEdit) GetByOk() (*string, bool) {
	if o == nil || IsNil(o.By) {
		return nil, false
	}
	return o.By, true
}

// HasBy returns a boolean if a field has been set.
func (o *ForumPostModelV1LastEdit) HasBy() bool {
	if o != nil && !IsNil(o.By) {
		return true
	}

	return false
}

// SetBy gets a reference to the given string and assigns it to the By field.
func (o *ForumPostModelV1LastEdit) SetBy(v string) {
	o.By = &v
}

// GetTime returns the Time field value if set, zero value otherwise.
func (o *ForumPostModelV1LastEdit) GetTime() TimeV1 {
	if o == nil || IsNil(o.Time) {
		var ret TimeV1
		return ret
	}
	return *o.Time
}

// GetTimeOk returns a tuple with the Time field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ForumPostModelV1LastEdit) GetTimeOk() (*TimeV1, bool) {
	if o == nil || IsNil(o.Time) {
		return nil, false
	}
	return o.Time, true
}

// HasTime returns a boolean if a field has been set.
func (o *ForumPostModelV1LastEdit) HasTime() bool {
	if o != nil && !IsNil(o.Time) {
		return true
	}

	return false
}

// SetTime gets a reference to the given TimeV1 and assigns it to the Time field.
func (o *ForumPostModelV1LastEdit) SetTime(v TimeV1) {
	o.Time = &v
}

func (o ForumPostModelV1LastEdit) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ForumPostModelV1LastEdit) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.By) {
		toSerialize["by"] = o.By
	}
	if !IsNil(o.Time) {
		toSerialize["time"] = o.Time
	}
	return toSerialize, nil
}

type NullableForumPostModelV1LastEdit struct {
	value *ForumPostModelV1LastEdit
	isSet bool
}

func (v NullableForumPostModelV1LastEdit) Get() *ForumPostModelV1LastEdit {
	return v.value
}

func (v *NullableForumPostModelV1LastEdit) Set(val *ForumPostModelV1LastEdit) {
	v.value = val
	v.isSet = true
}

func (v NullableForumPostModelV1LastEdit) IsSet() bool {
	return v.isSet
}

func (v *NullableForumPostModelV1LastEdit) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableForumPostModelV1LastEdit(val *ForumPostModelV1LastEdit) *NullableForumPostModelV1LastEdit {
	return &NullableForumPostModelV1LastEdit{value: val, isSet: true}
}

func (v NullableForumPostModelV1LastEdit) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableForumPostModelV1LastEdit) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


